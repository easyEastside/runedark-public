...
class NRPickpocket(NRBot):
    def __init__(self):
        title = "Pickpocket"
        description = (
            "This bot steals from NPCs in OSNR. Position your character near a tagged"
            " NPC you wish to steal from. "
            + "Start bot with > 50% HP. If you risk attacking nearby NPCs via misclick,"
    ...
    def create_options(self):
        self.options_builder.add_slider_option(
            "running_time", "How long to run (minutes)?", 1, 360
        )
        self.options_builder.add_dropdown_option(
            "logout_on_friends", "Logout when friends are nearby?", ["Yes", "No"]
        )
        self.options_builder.add_dropdown_option(
            "pickpocket_option",
        ...
        self.options_builder.add_dropdown_option(
            "should_drop_inv", "Drop inventory?", ["Yes", "No"]
        )
        self.options_builder.add_slider_option(
            "protect_rows", "If dropping, protect rows?", 0, 6
        )
    ...
    def save_options(self, options: dict):  # sourcery skip: low-code-quality
        for option, res in options.items():
            if option == "running_time":
                ...
                self.log_msg(
                    f"Protecting first {self.protect_rows} row(s) when dropping"
                    " inventory."
                )
            else:
    ...
    def main_loop(self):  # sourcery skip: low-code-quality, use-named-expression
        ...
        while time.time() - start_time < end_time:
            ...
            while self.get_hp() < 50:
                food_indexes = api.get_inv_item_indices(iid.all_food)
                if food_indexes:
                    self.log_msg("Eating...")
                    self.mouse.move_to(
                        self.win.inventory_slots[food_indexes[0]].random_point()
                    )
                    self.mouse.click()
                    if len(food_indexes) > 1:  # eat another if available
                        time.sleep(1)
                        self.mouse.move_to(
                            self.win.inventory_slots[food_indexes[1]].random_point()
                        )
                        self.mouse.click()
                else:
                    self.__logout(
                        "Out of food. Bot ran for"
                        f" {(time.time() - start_time) / 60} minutes."
                    )

            # Check if we should drop inventory
            if self.should_drop_inv and api.is_inv_full():
                skip_slots = api.get_inv_item_indices(iid.all_food)
                # Always drop the last row
                remove = range(24, 28)
                for index in remove:
                    if index in skip_slots:
                        skip_slots.remove(index)
                self.drop_all_items(skip_rows=self.protect_rows, skip_slots=skip_slots)

            # Steal from NPC
            npc_pos: RuneLiteObject = self.get_nearest_tag(self.cp.hsv.CYAN)
            if npc_pos is not None:
                self.mouse.move_to(npc_pos.random_point(), mouseSpeed="fastest")
                if not self.get_mouseover_text(contains="option"):
                ...
                if self.pickpocket_option != 0:
                    ...
                    if self.pickpocket_option == 1:
                        ...
                        delta_y = 56
                    self.mouse.move_rel(
                        x=5, y=delta_y, dx=25, dy=4, mouseSpeed="fastest"
                    )
                self.mouse.click()
                if self.pickpocket_option == 0:
                    time.sleep(0.3)
                ...
                if npc_search_fail_count > 39:
                    self.__logout(
                        f"No NPC found for {npc_search_fail_count} seconds. Bot ran for"
                        f" {(time.time() - start_time) / 60} minutes."
                    )

            # Click coin pouch
            stack_size = api.get_inv_item_stack_amount(iid.coin_pouches)
            if self.should_click_coin_pouch and stack_size > 22:
                self.log_msg("Clicking coin pouch...")
                pouch = imsearch.search_img_in_rect(
                    img=coin_pouch_path, rect=self.win.control_panel
                )
                if pouch:
                    self.mouse.move_to(
                        pouch.random_point(),
                    ...
                    if no_pouch_count > 5:
                        ...
                        self.drop_all_items(skip_rows=self.protect_rows)
                        no_pouch_count = 0

            # Check for mods
            if self.logout_on_friends and self.friends_nearby():
                self.__logout(
                    "Friends detected nearby. Bot ran for"
