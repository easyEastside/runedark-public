...
def trunc_norm_samp(lo: float, hi: float, mean=None, std=None) -> float:
    ...
    Note that a stadard deviation of 1/6 the mean is recommended. This allows for the
...
def trunc_chisquared_samp(df: int, min: float = 0, max: float = np.inf) -> float:
    """Generate a random sample from a Chisquared distribution.
    ...
    Returns:
        float: A random float from a Chisquared distribution.
